...
use database::save_config_to_mongodb;
...
async fn cron_job() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(config) = cursor.try_next().await? {
        process_user_config(&config, &bot_token).await?;
    }

    Ok(())
}

async fn process_user_config(config: &TrelloConfig, bot_token: &str) -> Result<(), Box<dyn std::error::Error>> {
    match check_trello_lists(&config.key, &config.token, &config.board).await {
        Ok(result) => {
            ...
            let message = generate_report_message(&todo_cards, &doing_cards, &done_cards);
