...
use database::{save_config_to_mongodb, save_reported_task, is_task_reported, log_error};
...
async fn connect_to_mongodb() -> Result<MongoClient, CustomError> {
    let connection_string = env::var("MONGODB_CONNECTION_STRING")
        .map_err(|e| CustomError(format!("Failed to get MONGODB_CONNECTION_STRING: {}", e)))?;
    let client_options = ClientOptions::parse(&connection_string)
        .await
        .map_err(|e| CustomError(format!("Failed to parse client options: {}", e)))?;
    MongoClient::with_options(client_options)
        .map_err(|e| CustomError(format!("Failed to create MongoDB client: {}", e)))
}

async fn process_user_config(config: &TrelloConfig, bot_token: &str, client: &MongoClient) -> Result<(), Box<dyn Error + Send + Sync>> {
    match check_trello_lists(&config.key, &config.token, &config.board).await {
        Ok(result) => {
            ...
            for card in &done_cards {
                if !is_task_reported(client, &card.id).await.map_err(|e| CustomError(e.to_string()))? {
                    new_done_cards.push(card);
                    save_reported_task(client, &card.id).await.map_err(|e| CustomError(e.to_string()))?;
                }
            }
            ...
            if !new_done_cards.is_empty() || !doing_cards.is_empty() || !todo_cards.is_empty() {
                ...
                send_message(bot_token, config.user_id, &message).await.map_err(|e| CustomError(e.to_string()))?;
            }
        }
        Err(err) => {
            let error_message = format!("Error for user {}: {}", config.user_id, err);
            log_error(client, &error_message).await.map_err(|e| CustomError(e.to_string()))?;
            let user_message = "Lỗi: Không thể tìm thấy đủ 3 danh sách 'ToDo', 'Doing' và 'Done' trong bảng Trello của bạn. Vui lòng thiết lập các danh sách này để bot hoạt động chính xác.";
            send_message(bot_token, config.user_id, user_message).await.map_err(|e| CustomError(e.to_string()))?;
        }
    }
    Ok(())
}
...
async fn cron_job() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    while let Some(config) = cursor.try_next().await? {
        if let Err(e) = process_user_config(&config, &bot_token, &client).await {
            let error_message = format!("Error processing user config: {:?}", e);
            log_error(&client, &error_message).await.map_err(|e| CustomError(e.to_string()))?;
        }
    }
